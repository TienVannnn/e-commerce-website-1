                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('permissions.update', $permission -> id) }}"> 
                        ...
                        <div class="mb-3">
                          ...
                          <input type="text" class="form-control" onkeyup="ChangeToSlug()" id="slug" name="name" value="{{ $permission -> name }}">
                        </div>
                        <div class="mb-3">
                            <label for="convert_slug" class="form-label">Key</label>
                            <input type="text" class="form-control" id="convert_slug" name="keycode" value="{{ $permission -> keycode}}">
                        </div>
                          <div class="mb-3">
                            <label for="parent" class="form-label">Parent permissions</label>
                            <select name="parent_id" id="parent" class="form-select">
                                <option value="0" {{ $permission -> parent_id == 0 ? 'selected' : '' }}>Parent permissions</option>
                                {!! $htmlOptions !!}
                            </select>
                          </div>
                        <div class="mb-3">
                            <label for="content" class="form-label">Description</label>
                            <textarea name="description" id="content"class="form-control" cols="30" rows="10">{{ $permission -> description }}</textarea>
                          </div>
                          <div class="mb-3">
                            <label>Active</label>
                            <div class="custom-control custom-radio">
                                <input class="custom-control-input" value="1" type="radio" id="active1" name="active" checked=""
                                {{ $permission -> active == 1 ? 'checked' : '' }}>
                                <label for="active1" class="custom-control-label">Yes</label>
                            </div>
                            <div class="custom-control custom-radio">
                                <input class="custom-control-input" value="0" type="radio" id="no_active" name="active" 
                                {{ $permission -> active == 0 ? 'checked' : '' }}>
